...
The 5 September parity implementation supersedes older blanket no-write, no-user-AI and mobile-only garment-capture statements in this historical brief. The current website permits only the bounded owner actions implemented by protected routes: supported profile and garment metadata updates, garment deletion with dependency cleanup, outfit save/today/plan/wear/removal, coupon redemption, notifications, sharing, shared-quota AI styling, Single Garment capture and up-to-five-photo Auto Extract. Web garment intake uses authenticated Oracle extraction, review-before-save, owner-scoped R2 upload and R2-before-Firestore persistence. Existing-image replacement, tap refinement, browser on-device ONNX fallback, gallery background jobs and Closet Quest submissions remain mobile-only.
