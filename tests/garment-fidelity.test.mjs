...
const complete={category:'Bottom',subCategory:'Joggers',colorDetail:'Dark blue teal',fabricTexture:'Matte woven slub',technical3DDetails:tech,visualProfile:{version:1,sourceFingerprint:'a'.repeat(64),colors:[{role:'base',hex:'#16464C',name:'Dark teal'}]}};
test('specific categories survive broad roles and unfamiliar types do not silently become shirts',()=>{
  ...
  for(const category of ['Bag','Skirt','Saree','Jumpsuit','Unknown','Bottom'])assert.equal(ghostCategory({category}),null);
...
test('straight hem and actual fabric survive; stale pink title and generic material cannot override pixels',()=>{
  ...
  assert.match(result.prompt,/#16464C/);assert.match(result.prompt,/Dark blue teal/);
