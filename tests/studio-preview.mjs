...
document.querySelector('#upload').insertAdjacentHTML('beforeend',hangerLoaderMarkup());updateHangerLoader(document.querySelector('#upload'),'Inspecting color, fabric and construction…');renderGarmentEvidence(document.querySelector('#review'),evidence);
...
document.getElementById('run').onclick=async()=>{
  ...
  try{
    ...
    renderGarmentEvidence(review,{title:'Next item'});check(review.querySelector('#upload-color-detail').value==='','next item does not inherit previous color detail');renderGarmentEvidence(review,evidence);
    ...
    const saved=counts.save;open('reject');await until(()=>document.querySelector('.ghost-studio').getAttribute('aria-busy')==='false');check(counts.save===saved,'quality failure prevents persistence');
