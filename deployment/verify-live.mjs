...
const paths=['index.html','app.js','companion.css','garment-upload.mjs','garment-appearance.mjs','garment-palette.mjs','garment-taxonomy.mjs','garment-progress.mjs','garment-review.mjs','garment-studio.css','ghost-contract.mjs','ghost-mannequin.mjs','ghost-studio.mjs','ghost-ui.mjs','outfit-intake.mjs','footwear-outline.mjs','full-look.mjs','downloads/clothmatics_ghost_v9.py','downloads/clothmatics_ghost_v9.ipynb','downloads/clothmatics_ghost_v9_2.py','downloads/clothmatics_ghost_v9_2.ipynb','downloads/clothmatics_ghost_v9_3.py','downloads/clothmatics_ghost_v9_3.ipynb','downloads/clothmatics_ghost_v9_4.py','downloads/clothmatics_ghost_v9_4.ipynb'];
...
const kaggleSourceMatches=kaggleSource.ok&&hash(kaggleSourceBytes)===hash(await readFile('kaggle/clothmatics_ghost_v9_4.py'));
...
const outfitGate=await fetch(origin+'/api/wardrobe/outfit-ghost',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
console.log(JSON.stringify({pagesOutfitGhostUnauthenticatedStatus:outfitGate.status}));
const fullLookGate=await fetch(origin+'/api/wardrobe/full-look',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
console.log(JSON.stringify({pagesFullLookUnauthenticatedStatus:fullLookGate.status}));
...
if(results.some(row=>!row.exactMatch)||!kaggleSourceMatches||gate.status!==401||outfitGate.status!==401||fullLookGate.status!==401)process.exitCode=1;
